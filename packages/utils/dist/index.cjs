...
var getLogsRetryHelper = ({
  ...
  if (match !== null) {
    const currentRange = _viem.hexToBigInt.call(void 0, params[0].toBlock) - _viem.hexToBigInt.call(void 0, params[0].fromBlock);
    const ranges = chunk({
      params,
  ...
  if (
    ...
    /response size should not greater than \d+ bytes/.test(sError) || // drpc
