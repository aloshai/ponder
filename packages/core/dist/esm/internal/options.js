...
export const buildOptions = ({ cliOptions }) => {
    ...
    return {
        ...
        maxThreads: process.env.PONDER_MAX_THREADS !== undefined
            ? Number(process.env.PONDER_MAX_THREADS)
            : 4,
        factoryAddressCountThreshold: 1000,
        truncationThreshold: process.env.PONDER_TRUNCATION_THRESHOLD !== undefined
            ? Number(process.env.PONDER_TRUNCATION_THRESHOLD)
            : 500,
        rpcMaxConcurrency: 256,
        addressBatchSize: process.env.PONDER_ADDRESS_BATCH_SIZE !== undefined
