...
export function buildLogFactory({ chainId, sourceId, fromBlock, toBlock, ...factoryConfig }) {
    let address;
    ...
    let childAddressLocation;
    if ("parameter" in factoryConfig) {
        const params = factoryConfig.parameter.split(".");
        const isParameterIndexedTopic = factoryConfig.event.inputs.some((input) => input.indexed === true && input.name === params[0]);
        if (params.length === 1 && isParameterIndexedTopic) {
            ...
            const nonIndexedInputs = factoryConfig.event.inputs.filter((x) => !("indexed" in x && x.indexed));
            const nonIndexedInputPosition = nonIndexedInputs.findIndex((input) => input.name === params[0]);
            if (nonIndexedInputPosition === -1) {
                throw new Error(`Factory event parameter not found in factory event signature. Got '${factoryConfig.parameter}', expected one of [${factoryConfig.event.inputs
                    .map((i) => `'${i.name}'`)
                    .join(", ")}].`);
            }
        ...
        if (factoryConfig.location !== "topic1" &&
            ...
            throw new Error(`Factory location is invalid. Got '${factoryConfig.location}', expected 'topic1', 'topic2', 'topic3', or 'offset[number]'.`);
        }
