...
import { PONDER_CHECKPOINT_TABLE_NAME, PONDER_META_TABLE_NAME, SCHEMATA, createDatabase, getPonderMetaTable, } from '../../database/index.js';
import { getLiveQueryChannelName, getLiveQueryNotifyProcedureName, getLiveQueryTempTableName, getViewsLiveQueryNotifyTriggerName, } from '../../drizzle/onchain.js';
...
export async function createViews({ cliOptions, }) {
    ...
    const meta = (await database.adminQB.wrap((db) => db
    ...
    await database.adminQB.wrap((db) => db.execute(`CREATE SCHEMA IF NOT EXISTS "${cliOptions.viewsSchema}"`));
    ...
    for (const table of meta[0].app.table_names) {
        await database.adminQB.wrap((db) => db.execute(`DROP VIEW IF EXISTS "${cliOptions.viewsSchema}"."${table}"`));
        await database.adminQB.wrap((db) => db.execute(`CREATE VIEW "${cliOptions.viewsSchema}"."${table}" AS SELECT * FROM "${cliOptions.schema}"."${table}"`));
    }
    if ("view_names" in meta[0].app) {
        for (const view of meta[0].app.view_names) {
            await database.adminQB.wrap((db) => db.execute(`DROP VIEW IF EXISTS "${cliOptions.viewsSchema}"."${view}"`));
            await database.adminQB.wrap((db) => db.execute(`CREATE VIEW "${cliOptions.viewsSchema}"."${view}" AS SELECT * FROM "${cliOptions.schema}"."${view}"`));
        }
    }
    logger.warn({
        ...
        count: meta[0].app.table_names.length +
            (meta[0].app.view_names?.length ?? 0),
        duration: endClock(),
    });
    await database.adminQB.wrap((db) => db.execute(sql.raw(`DROP VIEW IF EXISTS "${cliOptions.viewsSchema}"."${PONDER_META_TABLE_NAME}"`)));
    await database.adminQB.wrap((db) => db.execute(sql.raw(`DROP VIEW IF EXISTS "${cliOptions.viewsSchema}"."${PONDER_CHECKPOINT_TABLE_NAME}"`)));
    await database.adminQB.wrap((db) => db.execute(sql.raw(`CREATE VIEW "${cliOptions.viewsSchema}"."${PONDER_META_TABLE_NAME}" AS SELECT * FROM "${cliOptions.schema}"."${PONDER_META_TABLE_NAME}"`)));
    await database.adminQB.wrap((db) => db.execute(sql.raw(`CREATE VIEW "${cliOptions.viewsSchema}"."${PONDER_CHECKPOINT_TABLE_NAME}" AS SELECT * FROM "${cliOptions.schema}"."${PONDER_CHECKPOINT_TABLE_NAME}"`)));
    const notifyProcedure = getLiveQueryNotifyProcedureName();
    const channel = getLiveQueryChannelName(cliOptions.viewsSchema);
    await database.adminQB.wrap((db) => db.execute(`
