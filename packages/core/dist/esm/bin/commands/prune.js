...
import { PONDER_CHECKPOINT_TABLE_NAME, PONDER_META_TABLE_NAME, PONDER_STATUS_TABLE_NAME, VIEWS, createDatabase, getPonderMetaTable, } from '../../database/index.js';
import { TABLES } from '../../database/index.js';
import { getLiveQueryNotifyProcedureName, getLiveQueryProcedureName, getReorgProcedureName, getReorgTableName, } from '../../drizzle/onchain.js';
...
export async function prune({ cliOptions }) {
    ...
    const filteredResults = result.filter((row) => "is_dev" in row.value && row.value.is_dev === 0);
    ...
    if (tablesToDrop.length > 0) {
        await database.adminQB.wrap((db) => db.execute(`DROP TABLE IF EXISTS ${tablesToDrop.join(", ")} CASCADE`));
        logger.warn({
    ...
    if (viewsToDrop.length > 0) {
        await database.adminQB.wrap((db) => db.execute(`DROP VIEW IF EXISTS ${viewsToDrop.join(", ")} CASCADE`));
        logger.warn({
    ...
    if (functionsToDrop.length > 0) {
        await database.adminQB.wrap((db) => db.execute(`DROP FUNCTION IF EXISTS ${functionsToDrop.join(", ")} CASCADE`));
        logger.warn({
    ...
    if (schemasToDrop.length > 0) {
        await database.adminQB.wrap((db) => db.execute(`DROP SCHEMA IF EXISTS ${schemasToDrop.join(", ")} CASCADE`));
        logger.warn({
