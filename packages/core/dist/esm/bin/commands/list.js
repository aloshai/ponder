...
import { PONDER_META_TABLE_NAME, VIEWS, createDatabase, getPonderMetaTable, } from '../../database/index.js';
...
export async function list({ cliOptions }) {
    ...
    const rows = filteredResults.map((row) => ({
        ...
        relation_count: (row.value.table_names?.length ?? 0) +
            (row.value.view_names?.length ?? 0),
