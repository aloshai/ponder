...
export async function isolatedWorker({ options, namespaceBuild, crashRecoveryCheckpoint, chainIds, }) {
    ...
    options.indexingCacheMaxBytes = Math.floor(options.indexingCacheMaxBytes / indexingBuildResult.result.chains.length);
    options.rpcMaxConcurrency = Math.floor(options.rpcMaxConcurrency / indexingBuildResult.result.chains.length);
    options.syncEventsQuerySize = Math.floor(options.syncEventsQuerySize / indexingBuildResult.result.chains.length);
    ...
    await Promise.all(chainIds.map(async (chainId) => {
        const chainIndex = indexingBuildResult.result.chains.findIndex((c) => c.id === chainId);
        const indexingBuild = {
...
//# sourceMappingURL=isolatedWorker.js.map