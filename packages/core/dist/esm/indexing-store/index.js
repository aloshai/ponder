...
import { DbConnectionError, InvalidStoreAccessError, InvalidStoreMethodError, NonRetryableUserError, RawSqlError, RecordNotFoundError, RetryableError, UndefinedTableError, UniqueConstraintError, } from '../internal/errors.js';
...
export const createIndexingStore = ({ common, schemaBuild: { schema }, indexingCache, indexingErrorHandler, chainId, }) => {
    let qb = undefined;
    ...
    return {
        db: {
                ...
                return {
                    values: (userValues) => {
                        ...
                        const inner = {
                                ...
                                if (Array.isArray(ponderValues)) {
                                    ...
                                    for (const value of ponderValues) {
                                        ...
                                        if (row) {
                                            ...
                                            ponderRows.push(indexingCache.set({
                                                table,
                                                key: value,
                                                row: value,
                                                isUpdate: false,
                                    ...
                                    const userRows = ponderRows.map((row) => row === null ? row : copyOnWrite(row));
                                    return userRows;
                                }
                                else {
                            ...
                            onConflictDoUpdate: storeMethodWrapper(async (userUpdateValues) => {
                                common.metrics.ponder_indexing_store_queries_total.inc({
                                    table: getTableName(table),
                                    method: "insert",
                                });
                                    ...
                                    for (const value of userValues) {
                                        ...
                                        if (ponderRowUpdate) {
                                            if (typeof userUpdateValues === "function") {
                                                ...
                                                validateUpdateSet(table, ponderSet, ponderRowUpdate, primaryKeyCache);
                            ...
                            then: (onFulfilled, onRejected) => storeMethodWrapper(async () => {
                                ...
                                if (Array.isArray(ponderValues)) {
                                    ...
                                    const userRows = ponderRows.map((row) => row === null ? row : copyOnWrite(row));
                                    return Promise.resolve(userRows).then(onFulfilled, onRejected);
                                }
                                else {
                                    checkTableAccess(table, "insert", ponderValues, chainId);
                                    let ponderRow;
            ...
            update(table, key) {
                return {
                    set: storeMethodWrapper(async (userValues) => {
                        common.metrics.ponder_indexing_store_queries_total.inc({
                            table: getTableName(table),
                            method: "update",
                        });
            ...
            sql: drizzle(storeMethodWrapper(async (_sql, params, method, typings) => {
                ...
                if (isSelectOnly === false) {
                    ...
                    if (Array.from(relations ?? []).some((refName) => views.some((view) => getViewName(view) === refName))) {
                        await indexingCache.flush();
                    }
                    else {
                ...
                try {
                    ...
                    return await qb.transaction(async (tx) => {
                        const result = await tx.wrap((tx) => tx._.session
                    ...
                    throw new RawSqlError(error.message);
                }
                finally {
