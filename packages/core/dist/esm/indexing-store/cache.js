...
import { getCacheKey, getPrimaryKeyCache, getWhereCondition, normalizeRow, } from "./utils.js";
...
export const createIndexingCache = ({ common, schemaBuild: { schema }, crashRecoveryCheckpoint, eventCount, chainId, }) => {
    ...
    return {
        has({ table, key }) {
            ...
            return (cache.get(table).cache.has(ck) ??
        ...
        async get({ table, key }) {
            ...
            if (entry !== undefined) {
                if (cache.get(table).prefetched.has(ck) === false &&
            ...
            const result = await qb
                .wrap((db) => db.select().from(table).where(getWhereCondition(table, key)))
        ...
        async delete({ table, key }) {
            ...
            const inDb = await qb
                .wrap((db) => db.delete(table).where(getWhereCondition(table, key)).returning())
        ...
        async prefetch({ events }) {
            ...
            if (totalBytes > common.options.indexingCacheMaxBytes) {
                for (const table of tables.sort((a, b) => cache.get(a).diskReads - cache.get(b).diskReads)) {
                    if (cache.get(table).isCacheComplete === false)
                        continue;
                    totalBytes -= cache.get(table).bytes;
            ...
            for (const [table, tableCache] of cache) {
                ...
                for (const key of tableCache.cache.keys()) {
                    if (tableCache.spillover.has(key) ||
