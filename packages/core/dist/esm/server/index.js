...
export async function createServer({ common, database, apiBuild, }) {
    ...
    const hono = new Hono()
        ...
        if (chainParam) {
            ...
            return c.text(`Historical indexing is not complete for chain "${chainParam}".`, 503);
        }
        const isReady = await database.readonlyQB.wrap({ label: "select_ready" }, (db) => db
            .select()
        ...
        const status = {};
        const indexing = {};
        for (const { chainName, chainId, latestCheckpoint } of checkpoints.sort((a, b) => (a.chainId > b.chainId ? 1 : -1))) {
