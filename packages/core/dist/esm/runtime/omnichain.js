...
import { ZERO_CHECKPOINT_STRING, decodeCheckpoint, max, min, } from '../utils/checkpoint.js';
...
import { getHistoricalEventsOmnichain, refetchHistoricalEvents, } from "./historical.js";
...
export async function runOmnichain({ common, preBuild, namespaceBuild, schemaBuild, indexingBuild, crashRecoveryCheckpoint, database, }) {
    ...
    for (const chain of indexingBuild.chains) {
        ...
        const startBlock = Number.parseInt(syncProgress.start.number.replace("0x", ""), 16);
    ...
    const start = Number(decodeCheckpoint(getOmnichainCheckpoint({ perChainSync, tag: "start" }))
        .blockTimestamp);
    const end = Number(decodeCheckpoint(min(getOmnichainCheckpoint({ perChainSync, tag: "end" }), getOmnichainCheckpoint({ perChainSync, tag: "finalized" }))).blockTimestamp);
    for (const chain of indexingBuild.chains) {
    ...
    for await (const result of recordAsyncGenerator(getHistoricalEventsOmnichain({
        common,
        indexingBuild,
        crashRecoveryCheckpoint,
        perChainSync,
        ...
        let events = zipperMany(result.result.map(({ events }) => events), (a, b) => (a.checkpoint < b.checkpoint ? -1 : 1));
        ...
        await database.userQB.transaction(async (tx) => {
            const metricsSnapshot = await common.metrics.ponder_indexing_completed_events.get();
            ...
            try {
                    ...
                    updateIndexingSeconds(event) {
                        ...
                        for (const chain of indexingBuild.chains) {
                            common.metrics.ponder_historical_completed_indexing_seconds.set({ chain: chain.name }, Math.min(Math.max(Number(checkpoint.blockTimestamp) -
                                Math.max(seconds[chain.name].cached, seconds[chain.name].start), 0), Math.max(seconds[chain.name].end - seconds[chain.name].start, 0)));
                            common.metrics.ponder_indexing_timestamp.set({ chain: chain.name }, Math.max(Number(checkpoint.blockTimestamp), seconds[chain.name].end));
                        }
                    },
                });
                indexingStore.isProcessingEvents = false;
                ...
                await tx.wrap({ label: "update_checkpoints" }, (tx) => tx
                    .insert(PONDER_CHECKPOINT)
                    .values(result.result.map(({ chainId, checkpoint }) => ({
    ...
    for await (const event of bufferAsyncGenerator(getRealtimeEventsOmnichain({
        common,
        indexingBuild,
        perChainSync,
        database,
 ...
 */
export const getOmnichainCheckpoint = ({ perChainSync, tag, }) => {
    const checkpoints = Array.from(perChainSync.values()).map(({ syncProgress }) => syncProgress.getCheckpoint({ tag }));
    if (tag === "end") {
    ...
    // all chains.
    if (tag === "current") {
        const isComplete = Array.from(perChainSync.values()).map(({ syncProgress }) => syncProgress.isEnd());
...
//# sourceMappingURL=omnichain.js.map