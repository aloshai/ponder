...
import { getHistoricalEventsIsolated, refetchHistoricalEvents, } from "./historical.js";
...
export async function runIsolated({ common, preBuild, namespaceBuild, schemaBuild, indexingBuild, crashRecoveryCheckpoint, database, onReady, }) {
    ...
    common.metrics.ponder_historical_start_timestamp_seconds.set(label, startTimestamp);
    ...
    if (crashRecoveryCheckpoint === undefined) {
        await database.userQB.transaction(async (tx) => {
            ...
            const initialCheckpoint = min(syncProgress.getCheckpoint({ tag: "start" }), syncProgress.getCheckpoint({ tag: "finalized" }));
    ...
    for await (let { events, chainId, checkpoint, blockRange, } of recordAsyncGenerator(bufferAsyncGenerator(getHistoricalEventsIsolated({
        common,
        chain,
        indexingBuild,
        crashRecoveryCheckpoint,
    ...
    common.metrics.ponder_historical_completed_indexing_seconds.set(label, Math.max(seconds[chain.name].end -
        Math.max(seconds[chain.name].cached, seconds[chain.name].start), 0));
    common.metrics.ponder_indexing_timestamp.set({ chain: chain.name }, seconds[chain.name].end);
    ...
    for await (const event of bufferAsyncGenerator(getRealtimeEventsIsolated({
        common,
        indexingBuild,
        chain,
        syncProgress,
