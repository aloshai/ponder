...
import { decodeFunctionData, decodeFunctionResult, hexToBigInt, hexToNumber, toEventSelector, toFunctionSelector, } from "viem";
 ...
 */
export const buildEvents = ({ eventCallbacks, blocks, logs, transactions, transactionReceipts, traces, childAddresses, chainId, }) => {
    const events = [];
    const blockEventCallbackIndexes = [];
    const transactionEventCallbackIndexes = [];
    const logEventCallbackIndexes = [];
    const traceEventCallbackIndexes = [];
    const transferEventCallbackIndexes = [];
    for (let i = 0; i < eventCallbacks.length; i++) {
        const eventCallback = eventCallbacks[i];
        if (chainId !== eventCallback.filter.chainId)
            continue;
    ...
    for (const transaction of transactions) {
        ...
        while (blocksIndex < blocks.length &&
            Number(blocks[blocksIndex].number) < blockNumber) {
            blocksIndex++;
        }
        const block = blocks[blocksIndex];
        if (block === undefined) {
            throw new Error(`Failed to build events from block data. Missing block ${blockNumber} for chain ID ${chainId}`);
        }
        while (transactionReceiptsIndex < transactionReceipts.length &&
        ...
        for (const transactionEventCallbackIndex of transactionEventCallbackIndexes) {
            ...
            if (isTransactionFilterMatched({ filter, transaction }) &&
                ...
                if (filter.hasTransactionReceipt && transactionReceipt === undefined) {
                    throw new Error(`Failed to build events from block data. Missing transaction receipt for block ${blockNumber} and transaction index ${transactionIndex} for chain ID ${chainId}`);
                }
                events.push({
                    chainId: filter.chainId,
    ...
    for (const trace of traces) {
        ...
        let transaction;
        if (transactionsIndex < transactions.length &&
            transactions[transactionsIndex].blockNumber === blockNumber &&
            transactions[transactionsIndex].transactionIndex === transactionIndex) {
            transaction = transactions[transactionsIndex];
        }
        if (transaction === undefined) {
            throw new Error(`Failed to build events from block data. Missing transaction for block ${blockNumber} and transaction index ${transactionIndex} for chain ID ${chainId}`);
        }
        while (transactionReceiptsIndex < transactionReceipts.length &&
            (transactionReceipts[transactionReceiptsIndex].blockNumber <
                blockNumber ||
        ...
        if (transactionReceiptsIndex < transactionReceipts.length &&
            transactionReceipts[transactionReceiptsIndex].blockNumber ===
                blockNumber &&
            transactionReceipts[transactionReceiptsIndex].transactionIndex ===
                transactionIndex) {
        ...
        for (const traceEventCallbackIndex of traceEventCallbackIndexes) {
            const filter = eventCallbacks[traceEventCallbackIndex]
                .filter;
            if (isTraceFilterMatched({ filter, trace, block }) &&
                (isAddressFactory(filter.fromAddress)
                    ? isAddressMatched({
                        address: trace.from,
                        blockNumber,
        ...
        for (const transferEventCallbackIndex of transferEventCallbackIndexes) {
            const filter = eventCallbacks[transferEventCallbackIndex]
                .filter;
            if (isTransferFilterMatched({ filter, trace, block }) &&
                (isAddressFactory(filter.fromAddress)
                    ? isAddressMatched({
                        address: trace.from,
                        blockNumber,
    ...
    for (const log of logs) {
        ...
        for (const logEventCallbackIndex of logEventCallbackIndexes) {
            const filter = eventCallbacks[logEventCallbackIndex].filter;
...
export const decodeEvents = (common, chain, eventCallbacks, rawEvents) => {
    const events = [];
    const logDecodeFailureSelectors = new Set();
    let logDecodeFailureCount = 0;
    let logDecodeSuccessCount = 0;
    const traceDecodeFailureSelectors = new Set();
    ...
    for (const event of rawEvents) {
        ...
        if (eventCallback.type === "contract" &&
            ...
            try {
                ...
                const selector = toEventSelector(eventCallback.abiItem);
        ...
        else if (eventCallback.type === "contract" &&
            eventCallback.filter.type === "trace") {
            let decodedData;
            let decodedResult;
            try {
                decodedData = decodeFunctionData({
                    abi: [eventCallback.abiItem],
                ...
                const selector = toFunctionSelector(eventCallback.abiItem);
        ...
        else if (eventCallback.type === "account" &&
