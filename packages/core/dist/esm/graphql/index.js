...
import { Many, One, SQL, and, arrayContained, arrayContains, asc, count, createTableRelationsHelpers, desc, eq, extractTablesRelationalConfig, getTableColumns, gt, gte, ilike, inArray, is, isNotNull, isNull, like, lt, lte, ne, not, notIlike, notInArray, notLike, or, } from "drizzle-orm";
import { PgBigInt53, PgEnumColumn, PgInteger, PgSerial, PgView, getViewConfig, isPgEnum, isPgView, } from "drizzle-orm/pg-core";
...
export function buildGraphQLSchema({ schema, }) {
    ...
    const views = Object.entries(schema).filter((el) => isPgView(el[1]));
                ...
                for (const [columnName, column] of Object.entries(table.columns)) {
                    ...
                    if (type instanceof GraphQLScalarType ||
                        ...
                        if (["String", "ID"].includes(type.name)) {
                            const stringConditionSuffixes = column.columnType === "PgHex"
                                ? conditionSuffixes.string.filter((suffix) => suffix.endsWith("_nocase") === false)
    ...
    for (const [viewName, view] of views) {
        ...
        const filterType = new GraphQLInputObjectType({
            ...
            fields: () => {
                ...
                for (const [columnName, column] of Object.entries(viewConfig.selectedFields)) {
                    ...
                    if (is(relation, One)) {
                        ...
                        fieldConfigMap[relationName] = {
                            ...
                            resolve: (parent, args, context, info) => {
                                ...
                                return executePluralQuery(schema[referencedTable.tsName], referencedTable.columns, context.qb, args, includeTotalCount, relationalConditions);
                            },
                        };
                    }
                    else {
    ...
    for (const [viewName, view] of views) {
        ...
        entityPageTypes[viewName] = new GraphQLObjectType({
            ...
            fields: () => ({
                items: {
                    type: new GraphQLNonNull(new GraphQLList(new GraphQLNonNull(entityTypes[viewName]))),
                },
    ...
    for (const table of tables) {
        ...
        queryFields[pluralFieldName] = {
            ...
            resolve: async (_parent, args, context, info) => {
                const includeTotalCount = selectionIncludesField(info, "totalCount");
                return executePluralQuery(schema[table.tsName], table.columns, context.qb, args, includeTotalCount);
            },
        };
    }
    for (const [viewName, view] of views) {
        ...
        const singularFieldName = viewName.charAt(0).toLowerCase() + viewName.slice(1);
        const pluralFieldName = `${singularFieldName}s`;
        queryFields[pluralFieldName] = {
            type: new GraphQLNonNull(entityPageType),
            args: {
            ...
            resolve: async (_parent, args, context, info) => {
                const includeTotalCount = selectionIncludesField(info, "totalCount");
                return executePluralQuery(view, viewConfig.selectedFields, context.qb, args, includeTotalCount);
            },
        };
    }
    queryFields._meta = {
        ...
        resolve: async (_source, _args, context) => {
            ...
            const checkpoints = (await context.qb
                .wrap({ label: "select_checkpoints" }, (db) => db.execute("SELECT chain_name, chain_id, latest_checkpoint, safe_checkpoint from _ponder_checkpoint"))
                .then((res) => res.rows));
            const status = {};
            for (const { chain_name, chain_id, latest_checkpoint, } of checkpoints.sort((a, b) => (a.chain_id > b.chain_id ? 1 : -1))) {
                status[chain_name] = {
                    id: Number(chain_id),
                    block: {
...
async function executePluralQuery(relation, columns, qb, args, includeTotalCount, extraConditions = []) {
    ...
    if (after !== null) {
        ...
        const cursorCondition = buildCursorCondition(columns, orderBySchema, "after", cursorObject);
        const [rows, totalCount] = await Promise.all([
            qb.raw
                .select()
                .from(relation)
    ...
    const cursorCondition = buildCursorCondition(columns, orderBySchema, "before", cursorObject);
...
function buildOrderBySchema(relation, args) {
...
function buildCursorCondition(columns, orderBySchema, direction, cursorObject) {
...
export function buildDataLoaderCache(qb) {
