...
import { executeCopy, getCopyText, getQualifiedTableName, getQuotedColumnList, } from '../database/copy.js';
import { extractBlockNumberParam } from '../indexing/client.js';
import { getFilterFactories, isAddressFactory, unionFilterIncludeBlock, unionFilterIncludeTrace, unionFilterIncludeTransaction, unionFilterIncludeTransactionReceipt, } from '../runtime/filter.js';
import { encodeFragment, getFactoryFragments, getFragments, } from '../runtime/fragments.js';
import { intervalUnion } from '../utils/interval.js';
import { toLowerCase } from '../utils/lowercase.js';
import { orderObject } from '../utils/order.js';
import { startClock } from '../utils/timer.js';
import { and, asc, desc, eq, getTableName, gte, inArray, isNull, lt, lte, or, sql, } from "drizzle-orm";
...
export const createSyncStore = ({ common, qb, }) => {
    const COPY_THRESHOLD = 50;
    const copySyncInsert = async (table, rows, conflictColumns, label, context) => {
        ...
        await qb.transaction({ label }, async (tx) => {
            await tx.wrap((db) => db.execute(sql.raw(`CREATE TEMP TABLE IF NOT EXISTS "${tempName}" (LIKE ${qualifiedName} INCLUDING DEFAULTS)`)), context);
            await executeCopy(tx.$dialect, tx.$client, `"${tempName}" (${columnList})`, text);
            await tx.wrap((db) => db.execute(sql.raw(`INSERT INTO ${qualifiedName} SELECT * FROM "${tempName}" ON CONFLICT (${conflictClause}) DO NOTHING`)), context);
            await tx.wrap((db) => db.execute(sql.raw(`TRUNCATE "${tempName}"`)), context);
        }, undefined, context);
    };
    const syncStore = {
        insertIntervals: async ({ intervals, factoryIntervals, chainId }, context) => {
            if (intervals.length === 0 && factoryIntervals.length === 0)
                return;
            ...
            for (let i = 0; i < values.length; i += batchSize) {
                await qb.wrap({ label: "insert_intervals" }, (db) => db
        ...
        getIntervals: async ({ filters }, context) => {
            ...
            for (const filter of filters) {
                ...
                for (const fragment of fragments) {
                    queries.push(qb.raw
                        ...
                        .where(sql.raw(`fragment_id IN (${fragment.adjacentIds.map((id) => `'${id}'`).join(", ")})`))
                        .as("unnested")));
                }
                for (const factory of getFilterFactories(filter)) {
                    for (const fragment of getFactoryFragments(factory)) {
                        queries.push(qb.raw
                            ...
                            .from(PONDER_SYNC.intervals)
                            .where(sql.raw(`fragment_id = '${encodeFragment(fragment)}'`))
                            .as("unnested")));
                    }
                }
            }
            ...
            if (queries.length > 1) {
                ...
                for (let i = 0; i < queries.length; i += batchSize) {
                    const _rows = await qb.wrap({ label: "select_intervals" }, () => 
                    // @ts-expect-error
                    unionAll(...queries.slice(i, i + batchSize)), context);
                    if (i === 0) {
                        rows = _rows;
                    }
                    else {
                ...
                rows = await qb.wrap({ label: "select_intervals" }, () => queries[0].execute(), context);
            }
            ...
            for (const filter of filters) {
                ...
                for (const factory of getFilterFactories(filter)) {
                    ...
                    const filterIntervals = intervalUnion(result.get(filter).flatMap(({ intervals }) => intervals));
                    const factoryIntervals = intervalUnion(result.get(factory).flatMap(({ intervals }) => intervals));
        ...
        insertChildAddresses: async ({ factory, childAddresses, chainId }, context) => {
            if (childAddresses.size === 0)
                return;
            const { id, sourceId: _sourceId, ..._factory } = factory;
            const batchSize = Math.floor(common.options.databaseMaxQueryParameters / 3);
            const values = [];
            const factoryInsert = qb.raw.$with("factory_insert").as(qb.raw
            ...
            for (let i = 0; i < values.length; i += batchSize) {
                await qb.wrap({ label: "insert_child_addresses" }, (db) => db
                    .with(factoryInsert)
                    .insert(PONDER_SYNC.factoryAddresses)
                    .values(values.slice(i, i + batchSize)), context);
            }
        },
        getChildAddresses: ({ factory }, context) => {
            ...
            return qb
                .wrap({ label: "select_child_addresses" }, (db) => db
                .with(factoryInsert)
                .select({
                ...
                .where(eq(PONDER_SYNC.factoryAddresses.factoryId, qb.raw.select({ id: factoryInsert.id }).from(factoryInsert))), context)
                .then((rows) => {
                const result = new Map();
        ...
        getSafeCrashRecoveryBlock: async ({ chainId, timestamp }, context) => {
            const rows = await qb.wrap({ label: "select_crash_recovery_block" }, (db) => db
                .select({
        ...
        insertLogs: async ({ logs, chainId }, context) => {
            ...
            if (encoded.length > COPY_THRESHOLD) {
                await copySyncInsert(PONDER_SYNC.logs, encoded, ["chain_id", "block_number", "log_index"], "insert_logs_copy", context);
                return;
            }
            if (ENCODE_COLUMN_COUNTS.log === 0) {
                ENCODE_COLUMN_COUNTS.log = Object.keys(encoded[0]).length;
            }
            const batchSize = Math.floor(common.options.databaseMaxQueryParameters / ENCODE_COLUMN_COUNTS.log);
            for (let i = 0; i < encoded.length; i += batchSize) {
                await qb.wrap({ label: "insert_logs" }, (db) => db
        ...
        insertBlocks: async ({ blocks, chainId }, context) => {
            ...
            if (encoded.length > COPY_THRESHOLD) {
                await copySyncInsert(PONDER_SYNC.blocks, encoded, ["chain_id", "number"], "insert_blocks_copy", context);
                return;
            }
            if (ENCODE_COLUMN_COUNTS.block === 0) {
                ENCODE_COLUMN_COUNTS.block = Object.keys(encoded[0]).length;
            }
            const batchSize = Math.floor(common.options.databaseMaxQueryParameters / ENCODE_COLUMN_COUNTS.block);
            for (let i = 0; i < encoded.length; i += batchSize) {
                await qb.wrap({ label: "insert_blocks" }, (db) => db
        ...
        insertTransactions: async ({ transactions, chainId }, context) => {
            if (transactions.length === 0)
                return;
            const encoded = transactions.map((transaction) => encodeTransaction({ transaction, chainId }));
            if (encoded.length > COPY_THRESHOLD) {
                await copySyncInsert(PONDER_SYNC.transactions, encoded, ["chain_id", "block_number", "transaction_index"], "insert_transactions_copy", context);
                return;
            }
            ...
            for (let i = 0; i < encoded.length; i += batchSize) {
                await qb.wrap({ label: "insert_transactions" }, (db) => db
        ...
        insertTransactionReceipts: async ({ transactionReceipts, chainId }, context) => {
            if (transactionReceipts.length === 0)
                return;
            const encoded = transactionReceipts.map((transactionReceipt) => encodeTransactionReceipt({ transactionReceipt, chainId }));
            if (encoded.length > COPY_THRESHOLD) {
                await copySyncInsert(PONDER_SYNC.transactionReceipts, encoded, ["chain_id", "block_number", "transaction_index"], "insert_transaction_receipts_copy", context);
                return;
            }
            if (ENCODE_COLUMN_COUNTS.transactionReceipt === 0) {
                ENCODE_COLUMN_COUNTS.transactionReceipt = Object.keys(encoded[0]).length;
            }
            const batchSize = Math.floor(common.options.databaseMaxQueryParameters /
                ENCODE_COLUMN_COUNTS.transactionReceipt);
            for (let i = 0; i < encoded.length; i += batchSize) {
                await qb.wrap({ label: "insert_transaction_receipts" }, (db) => db
        ...
        insertTraces: async ({ traces, chainId }, context) => {
            if (traces.length === 0)
                return;
            const encoded = traces.map(({ trace, block, transaction }) => encodeTrace({ trace, block, transaction, chainId }));
            if (encoded.length > COPY_THRESHOLD) {
                await copySyncInsert(PONDER_SYNC.traces, encoded, ["chain_id", "block_number", "transaction_index", "trace_index"], "insert_traces_copy", context);
                return;
            }
            if (ENCODE_COLUMN_COUNTS.trace === 0) {
                ENCODE_COLUMN_COUNTS.trace = Object.keys(encoded[0]).length;
            }
            const batchSize = Math.floor(common.options.databaseMaxQueryParameters / ENCODE_COLUMN_COUNTS.trace);
            for (let i = 0; i < encoded.length; i += batchSize) {
                await qb.wrap({ label: "insert_traces" }, (db) => db
        ...
        getEventData: async ({ filters, fromBlock, toBlock, chainId, limit }, context) => {
            ...
            const shouldQueryTransactionReceipts = filters.some((filter) => filter.hasTransactionReceipt);
            ...
            const tracesQuery = qb.raw
                ...
                .orderBy(asc(PONDER_SYNC.traces.blockNumber), asc(PONDER_SYNC.traces.transactionIndex), asc(PONDER_SYNC.traces.traceIndex))
                .limit(limit);
            ...
            const [blocksRows, transactionsRows, transactionReceiptsRows, logsRows, tracesRows,] = await Promise.all([
                ...
                shouldQueryTransactions
                    ? qb.wrap({ label: "select_transactions" }, () => transactionsQuery, context)
                    : [],
                shouldQueryTransactionReceipts
                    ? qb.wrap({ label: "select_transaction_receipts" }, () => transactionReceiptsQuery, context)
                    : [],
                shouldQueryLogs
            ...
            for (let i = 0; i < transactionReceiptsRows.length; i++) {
                ...
                if (transactionReceipt.contractAddress) {
                    internalTransactionReceipt.contractAddress = toLowerCase(transactionReceipt.contractAddress);
                }
            ...
            return {
                ...
                transactionReceipts: transactionReceiptsRows,
                traces: tracesRows,
                cursor,
            };
        },
        insertRpcRequestResults: async ({ requests, chainId }, context) => {
            ...
            await qb.wrap({ label: "insert_rpc_requests" }, (db) => db
        ...
        getRpcRequestResults: async ({ requests, chainId }, context) => {
            ...
            const blockNumbersByRequest = new Array(requests.length);
            const requestHashes = new Array(requests.length);
            ...
            const blockNumbers = blockNumbersByRequest.filter((blockNumber) => blockNumber !== undefined);
            if (blockNumbers.length > 100) {
                const minBlockNumber = Math.min(...blockNumbers);
                const maxBlockNumber = Math.max(...blockNumbers);
                const nonBlockRequestHashes = requestHashes.filter((_, i) => blockNumbersByRequest[i] === undefined);
                const result = await Promise.all([
                    qb.wrap({ label: "select_rpc_requests" }, (db) => db
                        .select({
                        ...
                        .where(and(eq(PONDER_SYNC.rpcRequestResults.chainId, BigInt(chainId)), gte(PONDER_SYNC.rpcRequestResults.blockNumber, BigInt(minBlockNumber)), lte(PONDER_SYNC.rpcRequestResults.blockNumber, BigInt(maxBlockNumber)))), context),
                    nonBlockRequestHashes.length === 0
                        ...
                        : qb.wrap({ label: "select_rpc_requests" }, (db) => db
                            ...
                            .from(PONDER_SYNC.rpcRequestResults)
                            .where(and(eq(PONDER_SYNC.rpcRequestResults.chainId, BigInt(chainId)), inArray(PONDER_SYNC.rpcRequestResults.requestHash, nonBlockRequestHashes))), context),
                ]);
            ...
            const result = await qb.wrap({ label: "select_rpc_requests" }, (db) => db
                .select({
                request_hash: PONDER_SYNC.rpcRequestResults.requestHash,
                result: PONDER_SYNC.rpcRequestResults.result,
            })
                .from(PONDER_SYNC.rpcRequestResults)
                .where(and(eq(PONDER_SYNC.rpcRequestResults.chainId, BigInt(chainId)), inArray(PONDER_SYNC.rpcRequestResults.requestHash, requestHashes))), context);
            const results = new Map();
        ...
        pruneRpcRequestResults: async ({ blocks, chainId }, context) => {
            ...
            await qb.wrap({ label: "delete_rpc_requests" }, (db) => db
                .delete(PONDER_SYNC.rpcRequestResults)
                .where(and(eq(PONDER_SYNC.rpcRequestResults.chainId, BigInt(chainId)), inArray(PONDER_SYNC.rpcRequestResults.blockNumber, numbers))), context);
        },
        pruneByChain: async ({ chainId }, context) => qb.transaction(async (tx) => {
            await tx.wrap({ label: "delete_logs" }, (db) => db
                .delete(PONDER_SYNC.logs)
                .where(eq(PONDER_SYNC.logs.chainId, BigInt(chainId)))
                .execute(), context);
            await tx.wrap({ label: "delete_blocks" }, (db) => db
                .delete(PONDER_SYNC.blocks)
                .where(eq(PONDER_SYNC.blocks.chainId, BigInt(chainId)))
                .execute(), context);
            await tx.wrap({ label: "delete_traces" }, (db) => db
                .delete(PONDER_SYNC.traces)
                .where(eq(PONDER_SYNC.traces.chainId, BigInt(chainId)))
                .execute(), context);
            await tx.wrap({ label: "delete_transactions" }, (db) => db
                .delete(PONDER_SYNC.transactions)
                .where(eq(PONDER_SYNC.transactions.chainId, BigInt(chainId)))
                .execute(), context);
            await tx.wrap({ label: "delete_transaction_receipts" }, (db) => db
                .delete(PONDER_SYNC.transactionReceipts)
                .where(eq(PONDER_SYNC.transactionReceipts.chainId, BigInt(chainId)))
                .execute(), context);
            await tx.wrap({ label: "delete_factory_addresses" }, (db) => db
